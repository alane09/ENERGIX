...
export function MatriculeProvider({
  ...
  React.useEffect(() => {
    async function fetchMatricules() {
      try {
        ...
        if (selectedMatricules.length > 0) {
          const validMatricules = selectedMatricules.filter(m => matricules.includes(m))
    ...
    fetchMatricules()
  }, [selectedType, selectedYear, selectedMatricules, matricules])
  ...
  const value = React.useMemo(
    ...
    [selectedMatricules, matricules, toggleMatricule]
  )
              ...
              {selectedValues.length === 0 ? (
                ...
                selectedValues.map((matricule) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={(e) => removeMatricule(matricule, e)}
                      className="rounded-full h-3.5 w-3.5 flex items-center justify-center hover:bg-muted"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </Badge>
