...
const RegressionAPI = {
  async fetchRegressionData(vehicleType: string): Promise<RegressionData | null> {
    try {
      const response = await fetch(buildApiUrl(`/regression/monthly-totals/${vehicleType}`));
      if (!response.ok) {
  ...
  async performRegression(vehicleType: string): Promise<RegressionResult | null> {
    try {
      const response = await fetch(buildApiUrl(`/regression/perform/${vehicleType}`), {
        method: 'POST',
      });
      if (!response.ok) {
        throw new Error(`Error performing regression: ${response.statusText}`);
      }
      return await response.json();
    } catch (error) {
  ...
  async saveManualRegressionResult(data: RegressionResult): Promise<RegressionResult | null> {
    try {
      const response = await fetch(buildApiUrl('/regression/save-manual'), {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Error saving regression data: ${response.statusText}`);
      }
      return await response.json();
    } catch (error) {
  ...
  async getRegressionResultByType(vehicleType: string): Promise<RegressionResult | null> {
    try {
      const response = await fetch(buildApiUrl(`/regression/type/${vehicleType}`));
      if (!response.ok) {
  ...
  const fetchSERData = useCallback(async () => {
    ...
    try {
      ...
      if (regressionResult && Object.keys(regressionResult).length > 0) {
        ...
        // No regression data found, use mock data
        const mockData = generateMockMonthlyData(usedVehicleType);
        const transformedData = mockData.map((item: MonthlyDataPoint) => ({
          kilometrage: Number(item.kilometrage),
          tonnage: Number(item.tonnage),
          consommation: Number(item.consommation),
          month: item.month
        ...
        const mockRegressionData: RegressionData = {
          ...
          regressionEquation: "y = 0.2×Km + 10×Tonnage + 100",
          coefficients: {
            kilometrage: 0.2,
            tonnage: 10
          },
        ...
        // Generate regression lines
        const lines = generateRegressionLines(transformedData, mockRegressionData);
        if (lines) setRegressionLines(lines);
