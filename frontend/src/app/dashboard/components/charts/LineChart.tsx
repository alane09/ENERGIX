      ...
      return (
        <div className="bg-white dark:bg-gray-800 p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
          ...
          {isMultipleVehicleTypes ? (
              ...
              {payload.map((entry: {dataKey: string, stroke: string, value: number}, index: number) => {
                ...
                return (
                  <div key={index} className="flex justify-between items-center">
                    <span className="text-xs font-medium" style={{ color: entry.stroke }}>
                      {vehicleLabel}:
                    </span>
                    <span className="text-xs ml-2" style={{ color: entry.stroke }}>
            ...
            // Show single value
            <p className="text-sm" style={{ color: payload[0].stroke || 'var(--blue-600)' }}>
    ...
    <div className="w-full h-full">
      ...
      <ResponsiveContainer width="100%" height={height}>
        ...
        >
          ...
          <Legend />
          {selectedVehicleType === 'all' ? (
            // If 'all' is selected, show multiple lines with different colors
            ...
            // If a specific vehicle type is selected, show just one line
            <Line
              type="monotone"
              dataKey={dataKey}
              stroke={vehicleTypeColors[selectedVehicleType as keyof typeof vehicleTypeColors] || '#2563eb'}
              strokeWidth={2}
